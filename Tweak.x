...
static NSString* pendingNickName = nil;
...
static void updatePlayerListUI() {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        CGFloat py = 8;
        for (NSDictionary* player in capturedPlayers) {
            NSString* nick = player[@"nick"] ?: @"Unknown";
            NSString* display = parseDisplayName(nick);
            NSString* platform = parsePlatform(nick);
            NSNumber* actor = player[@"actor"] ?: @0;
            UIView* row = [[UIView alloc] initWithFrame:CGRectMake(8, py, 269, 44)];
            row.backgroundColor = COLOR_BTN;
            row.layer.cornerRadius = 8;
            NSString* platformIcon = [platform isEqualToString:@"VR"] ? @"🥽" :
                                     [platform isEqualToString:@"mobileIOS"] ? @"📱" : @"👤";
            UILabel* nameLbl = makeLabel([NSString stringWithFormat:@"%@ %@", platformIcon, display], CGRectMake(10, 4, 220, 18), 13, YES);
            UILabel* actorLbl = makeLabel([NSString stringWithFormat:@"Actor #%@  •  %@", actor, platform], CGRectMake(10, 24, 249, 14), 10, NO);
...
static UIView* buildPlayersTab(UIViewController* vc) {
    ...
    UILabel* placeholder = makeLabel(@"Join a lobby — players appear automatically", CGRectMake(10, 10, 265, 40), 12, NO);
...
static UIView* buildPacketTab(UIViewController* vc) {
    UIView* v = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 285, 340)];

    UILabel* hdr = makeLabel(@"Packet Log", CGRectMake(10, 8, 160, 18), 12, YES);
    hdr.textColor = COLOR_ACCENT;
    [v addSubview:hdr];

    UIButton* clearBtn = makeBtn(@"Clear", CGRectMake(220, 4, 56, 26));
    clearBtn.titleLabel.font = [UIFont systemFontOfSize:11];
    [clearBtn addTarget:vc action:NSSelectorFromString(@"clearPacketLog:") forControlEvents:UIControlEventTouchUpInside];
    [v addSubview:clearBtn];

    UITextView* log = [[UITextView alloc] initWithFrame:CGRectMake(8, 36, 269, 295)];
...
static void setupMenu() {
    ...
    packetLog = [NSMutableArray array];
    ...
    UILabel* version = makeLabel(@"V4", CGRectMake(200, 10, 36, 24), 12, YES);
    ...
    NSArray* tabNames = @[@"Players", @"Profile", @"Packets", @"RPC"];
    ...
    class_addMethod([vc class], NSSelectorFromString(@"clearPacketLog:"),
        imp_implementationWithBlock(^(id _self, UIButton* b){
            [packetLog removeAllObjects];
            UITextView* log = (UITextView*)[menuPanel viewWithTag:6001];
            log.text = @"Log cleared.";
        }), "v@:@");
}
